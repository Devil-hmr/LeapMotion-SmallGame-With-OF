...
bool Snake::isOutOfBounds(int xoff,int yoff,int screenWidth, int screenHeight){
	BodySegment head = body[0];
	if(y_direction){
		if( head.y > screenHeight || head.y < xoff)
			return true;
		else if((head.x + head.width) > screenWidth || (head.x < yoff))
			return true;
		else
			return false;
	}
	else{
		if(head.x > screenWidth || head.x < xoff){
			return true;
		}
		else if((head.y + head.height) > screenHeight || (head.y < yoff)){
			return true;
		}
		else{
			return false;
